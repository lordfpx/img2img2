...
export const ConversionItem = ({
	...
	const delta =
		convertedSize !== null ? item.originalSize - convertedSize : null;
	const gainRatio =
					...
					<div className="flex items-center gap-2">
						<label className="flex items-center gap-2 text-sm text-slate-300">
							...
							{item.usesGlobalFormat && (
								<span className="rounded-full border border-brand-500/30 bg-brand-500/10 px-2 py-0.5 text-[0.65rem] font-semibold uppercase tracking-wide text-brand-400">
									Global
								</span>
							)}
							<select
							...
							<div className="relative aspect-video">
								...
								{item.convertedUrl && (
									<img
										...
										style={{
											clipPath: `inset(0 ${100 - item.compareSplit}% 0 0)`,
										}}
									/>
								)}
								<input
									...
									onChange={(event) =>
										onSplitChange(item.id, Number(event.target.value))
									}
						...
						<div className="space-y-3 text-sm text-slate-300">
							<div className="flex items-center justify-between">
								...
								<span className="font-medium text-white">
									{item.file.type || "Détecté automatiquement"}
								</span>
							</div>
							<div className="flex items-center justify-between">
